/* =========================================================
   BLUEPOWER — silo-contact.js
   Silo krypties kontaktų forma. Tas pats mechanizmas kaip
   contact.js (vandens srities forma), bet BE "Contact reason"
   lauko — jo gyvame silo/contact puslapyje nėra (tai Rent/Buy
   pasirinkimas, aktualus tik vandens srities įrangos nuomai/
   pardavimui).

   Firmos paieška naudoja TIKRĄ Brønnøysundo registro API
   (data.brreg.no). Jei ji nepasiekiama — laukas veikia kaip
   paprastas teksto laukas, jokių išgalvotų rezultatų.

   Forma niekur nesiunčiama — žr. sendEnquiry() apačioje.
   ========================================================= */
(function () {
  'use strict';
  ...
  var S = { loc: 'no', co: '', orgnr: '', country: '',
            nm: '', role: '', ph: '', em: '', msg: '' };

  /* ---------- laukai ---------- */
  ['nm', 'role', 'ph', 'em', 'msg'].forEach(function (k) {
    var el = q('[name=' + k + ']');
  ...
  q('#ctSend').addEventListener('click', function () {
    ...
    var ref = 'BP-SC-2026-' + String(Math.floor(1000 + Math.random() * 9000));
    sendEnquiry({
      ref: ref, location: S.loc,
  ...
  function sendEnquiry(data) {
    console.log('SILO CONTACT — enquiry (not sent anywhere):', data);
